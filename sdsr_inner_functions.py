...
class STDR_for_Multiple_Alignments():
    ...
    def __init__(self, sequences_path, format_type, number_of_genes, min_percent_for_balanced_kmean, 
                inner_method, threshold, sum_or_median, path_to_exported_trees, 
                output_estimated_ml_trees_path, output_astral_file_name, cd_path_for_astral_exe, 
                different_num_of_genes_to_reconstruct=False, number_of_genes_for_reconstruction=None,
                partition_only = False, astral_bin = "astral4"):
        ...
        self.different_num_of_genes_to_reconstruct = different_num_of_genes_to_reconstruct
        self.number_of_genes_for_reconstruction = number_of_genes_for_reconstruction
    ...
    def process_multiple_genes(self):
        ...
        all_alignments = list(AlignIO.parse(self.sequences_path, 'phylip'))
    ...
    def process_whole_genome(self):
        ...
        selected_alignments = self.create_gene_windows_from_whole_genom(self.sequences_path, self.number_of_genes)
    ...
    def process_multiple_genes_from_different_files(self):
        ...
        for filename in os.listdir(self.sequences_path):
            name = os.path.join(self.sequences_path, filename)
    ...
    def create_similarity_matrices(self):
        ...
        n_workers = min(32, self.max_workers)
        print(f"Using {n_workers} CPU workers for creating similarirt matrices")
    ...
    def process_alignment(self, i):
        ...
        # Check the number of species from the alignment shape
        if self.alignments[i].shape[0] >= 1000:
    ...
    def apply_astral_without_parrelal_genes_reconstruction(self, node):
        ...
        path_ml_trees_linux = self.output_estimated_ml_trees_path + node.tree_id
        output_astral_file_name = self.output_astral_file_name + node.tree_id
        astral_dir = os.path.abspath(self.cd_path_for_astral_exe)
        command = fr'cd "{astral_dir}" && ./{self.astral_bin} -u 0 -i {path_ml_trees_linux} -o {output_astral_file_name}' #topology only
        #print(command)
        '''
        if self.type_of_run == 'pc':
    ...
    def apply_astral(self, node):
        ...
        output_astral_file_name = self.output_astral_file_name + node.tree_id
        astral_dir = os.path.abspath(self.cd_path_for_astral_exe)
        command = fr'cd "{astral_dir}" && ./{self.astral_bin} -u 0 -i {path_ml_trees_linux} -o {output_astral_file_name}'
    ...
    def reconstruct_alg_wrapper(self, node):
        ...
        if self.different_num_of_genes_to_reconstruct:
            self.alignments = self.alignments[ :self.number_of_genes_for_reconstruction]
            print(f'partition run on {self.number_of_genes} and recunstruction run on {int(self.number_of_genes_for_reconstruction)} genes')
    ...
    def create_folder_to_subtrees(self):
        """Create a temporary folder with a unique name for subtrees"""
        # Create a temporary directory with a descriptive prefix
        folder_export_subtrees = tempfile.mkdtemp(
            prefix=f"sdsr_subtrees_{self.inner_method}_",
        )
        print(f"Created temporary folder: {folder_export_subtrees}")
    ...
    def _reconstruct_single_tree(self, tree_id, data, folder_export_subtrees):
    ...
    def run_astral_for_subtree(self, tree_id, data, folder_export_subtrees):
        ...
        astral_dir = os.path.abspath(self.cd_path_for_astral_exe)
        command = f'cd "{astral_dir}" && ./{self.astral_bin} -u 0 -i {ml_trees_file} -o {output_astral_file}'
    ...
    def reconstruct_all_trees_for_single_marker_for_astral(self, marker_idx, trees_in_marker, folder_export_subtrees):
        """
    ...
    def reconstruct_single_gene_tree_for_subtree_for_astral(self, tree_id, data, alignment_idx, alignment, folder_export_subtrees):
        """
    ...
    def reconstruct_small_subtrees_parallel(self):
        ...
        if self.inner_method == 'astral':
            ...
            with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                for tree_id, data in self.nodes_to_reconstruct.items():
                    executor.submit(self._reconstruct_single_tree, tree_id, data, folder_export_subtrees)
